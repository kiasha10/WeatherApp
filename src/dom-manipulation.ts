  ...
  try {
    ...
    for (const area of areas) {
      try {
        ...
        console.error(`Failed to fetch weather data for area ${area.name}:`, error);
      }
    }
    ...
    if (viewed.length > 0) {
      ...
      viewed.forEach(async (area: Area) => {
        try {
          ...
          console.error(`Failed to fetch weather data for previously viewed area:`, error);
        }
      });
    ...
    async function manageSearch() {
      try {
        ...
        console.error("Failed to manage search:", error);
      }
    }
  } catch (error) {
    console.error("Failed to display areas:", error);
  }
}
...
export async function displayWeatherStats(
  ...
  try {
    ...
    console.error("Failed to display weather stats:", error);
  }
}
